...
class QueryBioThingsExplorer:
    ...
    @staticmethod
    def send_query_get(input-prefix, output-prefix, input-value):
        url = QueryBioThingsExplorer.API_BASE_URL.replace('input-prefix', input-prefix).replace('output-prefix', output-prefix).replace('input-value', input-value)
        try:
